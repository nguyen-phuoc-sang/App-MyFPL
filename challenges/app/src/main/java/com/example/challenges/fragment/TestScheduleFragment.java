...
public class TestScheduleFragment extends Fragment implements SearchableFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_testschedule, container, false);
        ...
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl("http://192.168.1.10/challenges/")
        ...
        call.enqueue(new Callback<List<LichThi>>() {
            ...
            public void onResponse(Call<List<LichThi>> call, Response<List<LichThi>> response) {
                if (response.isSuccessful() && response.body() != null){
                    List<LichThi> lichHocs = response.body();
                    lichThiList.clear();
                    lichThiList.addAll(lichHocs);
    ...
    public void performSearch(String query) {
        if (TextUtils.isEmpty(query)){
            ...
            for (LichThi lichthi : lichThiList){
                if (lichthi.getPhong().toLowerCase().contains(query.toLowerCase())
                        || lichthi.getCa_thi().toLowerCase().contains(query.toLowerCase())
                        || lichthi.getNgay_thi().toLowerCase().contains(query.toLowerCase())
